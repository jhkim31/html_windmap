...
function getVector(latitude, longitude) {                       
    if (latitude <= 33 || latitude >= 38) return [2,-2,2.8]             // 만약 위도 33 이하, 38 이상이면 1, -1 벡터 리턴
    if (longitude <= 124 || longitude >= 130) return [2,-2,2.8]         // 만약 경도 124 이하, 130 이상이면 1, -1 벡터 리턴
...
function selectGrid(latitude, longitude) {                                
    gridlng = parseInt((longitude - 124) / gap)         //ex) 
    gridlat = parseInt((38 - latitude) / gap)           //ex) 
